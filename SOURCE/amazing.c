...
int main(int argc, char **argv) {
    ...
    double ARE, MORRISTOWN, AMAZING, H, MEANINGLESS, C, J, NEW, Z, WIDTH, S, COMPUTING, CREATIVE, JERSEY, DIMENSIONS, TRY, ON, X, WHAT, V, PROGRAM, AGAIN, R, YOUR, LENGTH, I, Q;
    double W[1000] = {0}; /* Static array fallback */
    char _input_buf[256];
...
line_100:
    printf("%s", "WHAT ARE YOUR WIDTH AND LENGTH"); if (fgets(_input_buf, sizeof(_input_buf), stdin)) sscanf(_input_buf, "%lf", &H,V);
...
line_165:
    I = 1; for_loop_I: if ((1 >= 0 && I > H) || (1 < 0 && I < H)) goto end_for_I;
...
line_490:
    if (W(R,S+1)!=0) goto line_520;
    /* BASIC: 500 X=INT(RND(1)*2+1) */
line_500:
    X = floor(((double)rand() / (double)RAND_MAX)*2+1);
    /* BASIC: 510 ON X GOTO 790,910 */
line_510:
    /* UNTRANSLATED: ON X GOTO 790,910 */
    /* BASIC: 520 GOTO 790 */
line_520:
    goto line_790;
...
line_630:
    if (W(R,S+1)!=0) goto line_660;
    /* BASIC: 640 X=INT(RND(1)*2+1) */
line_640:
    X = floor(((double)rand() / (double)RAND_MAX)*2+1);
    /* BASIC: 650 ON X GOTO 820,910 */
line_650:
    /* UNTRANSLATED: ON X GOTO 820,910 */
    /* BASIC: 660 GOTO 820 */
line_660:
    goto line_820;
...
line_700:
    if (W(R,S+1)!=0) goto line_730;
    /* BASIC: 710 X=INT(RND(1)*2+1) */
line_710:
    X = floor(((double)rand() / (double)RAND_MAX)*2+1);
    /* BASIC: 720 ON X GOTO 860,910 */
line_720:
    /* UNTRANSLATED: ON X GOTO 860,910 */
    /* BASIC: 730 GOTO 860 */
line_730:
    goto line_860;
...
line_1010:
    J = 1; for_loop_J: if ((1 >= 0 && J > V) || (1 < 0 && J < V)) goto end_for_J;
    /* BASIC: 1011 PRINT "I"; */
line_1011:
    printf("%s", "I");
...
line_1045:
    if (V(I,J)==0) goto line_1060;
    /* BASIC: 1050 IF V(I,J)=2 THEN 1060 */
line_1050:
    if (V(I,J)==2) goto line_1060;
    /* BASIC: 1051 PRINT ":  "; */
line_1051:
    printf("%s", ":  ");
