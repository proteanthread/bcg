...
int main(int argc, char **argv) {
    ...
    double MINIMUM, ABS, BETTER, BASE, SHOOTING, TRAINING, GUNNER, GO, ROUNDS, NEW, NICE, OK, REFRESHER, JERSEY, THE, WITHIN, ON, A, GIVING, EXPENDED, FORT, IT, DISTANCE, CAMP, T, GUN, WERE, IN, DEGREES, PROJECTILE, CHARGE, BY, MORRISTOWN, SIGHTED, CREW, TARGET, DESTROY, THEM, OFFICER, SIN, TRY, BOOM, BACK, RANGE, S1, AMMUNITION, YOUR, I, HAS, DEGREE, Y, MORE, BEEN, IS, YARDS, OF, Z, DESTROYED, ACTIVITY, S, SILL, WILL, CREATIVE, B2, YOU, X, ENEMY, ESTIMATE, FORWARD, ONE, SHORT, ARE, B, ELEVATION, TELLING, TOTAL, ORDERS, COMPUTING, JUST, HIT, E, OBSERVER, PLACE, AGAIN, R, OVER, MAXIMUM, N, HAVE;
    char _input_buf[256];
...
line_290:
    printf("%g ", (double)("SHORT OF TARGET BY "fabs(E))); printf("%s", "YARDS."); printf("\n");
    /* BASIC: 300 GOTO 390 */
line_300:
    goto line_390;
...
line_370:
    printf("%g ", (double)("DISTANCE TO THE TARGET IS "T)); printf("%s", " YARDS."); printf("\n");
    /* BASIC: 380 PRINT */
line_380:
    printf("\n");
    /* BASIC: 390 PRINT */
...
line_500:
    printf("\n");
    printf("%s", "TRY AGAIN (Y OR N)"); fgets(Z_str, sizeof(Z_str), stdin); Z_str[strcspn(Z_str, "\n")] = 0;
    /* BASIC: 510 IF Z$="Y" THEN 170 */
line_510:
    if (Z$=="Y") goto line_170;
