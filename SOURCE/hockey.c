...
int main(int argc, char **argv) {
    ...
    double OPENING, MAKES, FACE, ENTER, SHOT, G2, LEADS, P, GAME, GO, CHR, HURT, J, NEW, TYPE, CORRESPONDING, GOAL, INSTRUCTIONS, COACH, ANSWER, RE, THEY, THE, JERSEY, REQUIRED, DEFENSEMAN, GOALS, ON, TRAILER, LEFT, A, STARTING, WRISTSHOT, CUTTING, CUTS, RIGHT, UP, IT, SHOTS, GOLFS, A1, CONTROL, LIKE, RIPS, MINUTES, T, GOALKEEPER, IN, HIM, HANGS, SITUATION, NUMBER, SLAP, NAMES, OUT, HAND, T3, BY, CLEARS, MORRISTOWN, SNAP, STEAMER, ANY, SCORING, ALONE, LOW, PLAYER, Z1, TEAMS, BOOMER, AREA, OFF, START, GOES, WING, CENTER, OH, SLAPSHOT, SAVE, STREAKING, BIG, PASSES, NET, WRIST, BACK, PAD, WHAT, HIMSELF, BETWEEN, DEFENSE, AIMING, GOALTENDER, STANDS, FINAL, WHACKS, SUMMARY, WANT, WHISTLES, LET, G1, S1, RED, ALL, S2, I, TONIGHTS, LINEUP, S3, YOUR, HE, RESPONSE, CROWD, FLIPS, HAS, THROUGH, FROM, LINE, POST, GLOVE, SKATE, LOOSE, H, IS, SHUFFLES, STICK, DROP, BACKHAND, KICK, SPECTACULAR, OF, Z, ONLY, B1, FLIP, SCORE, SCORED, ENTERED, GOD, T6, SAME, S, WHEEELING, WILL, SIREN, UPPER, CREATIVE, ORDER, LOWER, MY, HIS, YOU, BACKHANDS, X, COMES, AS, NO, RATTLES, NAME, WOULD, WITH, ONE, WE, OTHER, QUESTION, REFEREE, G, ASKED, T1, A2, PASSING, CLEARED, TEAM, ARE, BEAUTY, BE, FOLLOW, B, C, ASSISTED, MAKE, REBOUND, LONG, HEAD, EXPLANATORY, INTO, SNAPS, TWO, PERFECT, COVERS, COMPUTING, CORNER, READY, HOCKEY, UNASSISTED, PUCK, ASSISTS, SIDE, ICE, WAY, FLYING, GIVES, FIRES, SIMULATED, GETS, AT, L, KICKED, REBOUNDS, PASS, PLAYERS, DOWN, DO, R, YES, THAT, WHO, OVER, N, T2, HAVE, THIS, DROPS, HITS, PRETTY;
...
line_60:
    if (C$=="NO") goto line_90;
    /* BASIC: 65 IF C$="YES" THEN 80 */
line_65:
    if (C$=="YES") goto line_80;
...
line_100:
    ...
    printf("? "); if (fgets(_input_buf, sizeof(_input_buf), stdin)) sscanf(_input_buf, "%lf", &A$(7),B$(7));
    /* BASIC: 105 PRINT */
line_105:
    printf("\n");
...
line_120:
    if (T6<1) goto line_110;
    printf("\n");
    /* BASIC: 130 PRINT "WOULD THE " A$(7) " COACH ENTER HIS TEAM" */
line_130:
    printf("%s", "WOULD THE " A$(7) " COACH ENTER HIS TEAM"); printf("\n");
    /* BASIC: 135 PRINT */
line_135:
    printf("\n");
...
line_150:
    printf("%s", "WOULD THE " B$(7) " COACH DO THE SAME"); printf("\n");
    /* BASIC: 155 PRINT */
line_155:
    printf("\n");
    /* BASIC: 160 FOR T=1 TO 6:PRINT "PLAYER"T;:INPUT B$(T):NEXT T:PRINT */
line_160:
    T = 1; for_loop_T: if ((1 >= 0 && T > 6) || (1 < 0 && T < 6)) goto end_for_T;
    printf("%g ", (double)("PLAYER"T));
    printf("? "); if (fgets(_input_buf, sizeof(_input_buf), stdin)) sscanf(_input_buf, "%lf", &B$(T));
...
line_180:
    printf("\n");
    { int _t; for(_t=0; _t<10; _t++) printf(" "); } printf("%g ", (double)(A$(7) " STARTING LINEUP")); printf("\n");
    /* BASIC: 190 FOR T=1 TO 6:PRINT A$(T):NEXT T */
line_190:
    T = 1; for_loop_T: if ((1 >= 0 && T > 6) || (1 < 0 && T < 6)) goto end_for_T;
    printf("%g ", (double)(A$(T))); printf("\n");
    T += 1; goto for_loop_T; end_for_T:;
    /* BASIC: 200 PRINT:PRINT TAB(10);B$(7)" STARTING LINEUP" */
line_200:
    printf("\n");
    { int _t; for(_t=0; _t<10; _t++) printf(" "); } printf("%g ", (double)(B$(7)" STARTING LINEUP")); printf("\n");
    /* BASIC: 210 FOR T=1 TO 6:PRINT B$(T):NEXT T:PRINT */
line_210:
    T = 1; for_loop_T: if ((1 >= 0 && T > 6) || (1 < 0 && T < 6)) goto end_for_T;
    printf("%g ", (double)(B$(T))); printf("\n");
    T += 1; goto for_loop_T; end_for_T:;
    printf("\n");
...
line_230:
    printf("%g ", (double)(R$ " WILL DROP THE PUCK BETWEEN " A$(2) " && " B$(2))); printf("\n");
    /* BASIC: 240 FOR L=1 TO T6:IF L=1 THEN 260 */
line_240:
    L = 1; for_loop_L: if ((1 >= 0 && L > T6) || (1 < 0 && L < T6)) goto end_for_L;
...
line_260:
    C = floor(2*((double)rand() / (double)RAND_MAX)(X))+1;
    /* UNTRANSLATED: ON C GOTO 270,280 */
    /* BASIC: 270 PRINT A$(7) " HAS CONTROL OF THE PUCK":GOTO 290 */
line_270:
    printf("%g ", (double)(A$(7) " HAS CONTROL OF THE PUCK")); printf("\n");
    goto line_290;
    /* BASIC: 280 PRINT B$(7) " HAS CONTROL." */
line_280:
    printf("%g ", (double)(B$(7) " HAS CONTROL.")); printf("\n");
...
line_370:
    /* UNTRANSLATED: ON C GOTO 380,480 */
    /* BASIC: 380 PRINT A$(H(J-1));:G=H(J-1):G1=0:G2=0 */
line_380:
    printf("%g ", (double)(A$(H(J-1))));
    G = H(J-1);
    G1 = 0;
    G2 = 0;
    /* BASIC: 390 ON S GOTO 400,420,440,460 */
line_390:
    /* UNTRANSLATED: ON S GOTO 400,420,440,460 */
...
line_490:
    /* UNTRANSLATED: ON C GOTO 500,640 */
    /* BASIC: 500 ON P GOTO 510,540,570 */
line_500:
    /* UNTRANSLATED: ON P GOTO 510,540,570 */
    /* BASIC: 510 PRINT A$(H(J-2)) " LEADS " A$(H(J-1)) " WITH A PERFECT PASS." */
line_510:
    printf("%g ", (double)(A$(H(J-2)) " LEADS " A$(H(J-1)) " WITH A PERFECT PASS.")); printf("\n");
    /* BASIC: 520 PRINT A$(H(J-1)) " CUTTING IN!!!" */
line_520:
    printf("%g ", (double)(A$(H(J-1)) " CUTTING IN!!!")); printf("\n");
...
line_540:
    printf("%g ", (double)(A$(H(J-2)) " GIVES TO A STREAKING " A$(H(J-1)))); printf("\n");
    /* BASIC: 550 PRINT A$(H(J-3)) " COMES DOWN ON " B$(5) " AND " B$(4) */
line_550:
    printf("%g ", (double)(A$(H(J-3)) " COMES DOWN ON " B$(5) " && " B$(4))); printf("\n");
...
line_580:
    printf("%g ", (double)(A$(H(J-3)) " LEADS " A$(H(J-2)))); printf("\n");
    /* BASIC: 590 PRINT A$(H(J-2)) " IS WHEEELING THROUGH CENTER." */
line_590:
    printf("%g ", (double)(A$(H(J-2)) " IS WHEEELING THROUGH CENTER.")); printf("\n");
    /* BASIC: 600 PRINT A$(H(J-2)) " GIVES AND GOES WITH " A$(H(J-1)) */
line_600:
    printf("%g ", (double)(A$(H(J-2)) " GIVES && GOES WITH " A$(H(J-1)))); printf("\n");
    /* BASIC: 610 PRINT "PRETTY PASSING!" */
line_610:
    printf("%s", "PRETTY PASSING!"); printf("\n");
    /* BASIC: 620 PRINT A$(H(J-1)) " DROPS IT TO " A$(H(J-4)) */
line_620:
    printf("%g ", (double)(A$(H(J-1)) " DROPS IT TO " A$(H(J-4)))); printf("\n");
...
line_640:
    /* UNTRANSLATED: ON P GOTO 650,670,720 */
    /* BASIC: 650 PRINT B$(H(J-1)) " HITS " B$(H(J-2)) " FLYING DOWN THE LEFT SIDE" */
line_650:
    printf("%g ", (double)(B$(H(J-1)) " HITS " B$(H(J-2)) " FLYING DOWN THE LEFT SIDE")); printf("\n");
...
line_680:
    printf("%s", "ONLY " A$(4) " AND " A$(5) " ARE BACK."); printf("\n");
    /* BASIC: 690 PRINT B$(H(J-2)) " GIVES OFF TO " B$(H(J-1)) */
line_690:
    printf("%g ", (double)(B$(H(J-2)) " GIVES OFF TO " B$(H(J-1)))); printf("\n");
    /* BASIC: 700 PRINT B$(H(J-1)) " DROPS TO " B$(H(J-3)) */
line_700:
    printf("%g ", (double)(B$(H(J-1)) " DROPS TO " B$(H(J-3)))); printf("\n");
...
line_730:
    printf("%g ", (double)(B$(H(J-4)) " GIVES TO " B$(H(J-2)) " WHO SHUFFLES IT OFF TO")); printf("\n");
    /* BASIC: 740 PRINT B$(H(J-1)) " WHO FIRES A WING TO WING PASS TO " */
line_740:
    printf("%g ", (double)(B$(H(J-1)) " WHO FIRES A WING TO WING PASS TO ")); printf("\n");
    /* BASIC: 750 PRINT B$(H(J-3)) " AS HE CUTS IN ALONE!!" */
line_750:
    printf("%g ", (double)(B$(H(J-3)) " AS HE CUTS IN ALONE!!")); printf("\n");
...
line_780:
    /* UNTRANSLATED: ON C GOTO 790,880 */
    /* BASIC: 790 PRINT A$(G);:ON S GOTO 800,820,840,860 */
line_790:
    printf("%g ", (double)(A$(G)));
    /* UNTRANSLATED: ON S GOTO 800,820,840,860 */
...
line_900:
    /* UNTRANSLATED: ON C GOTO 910,920 */
...
line_930:
    A1 = floor(4*((double)rand() / (double)RAND_MAX)(X))+1;
    if (A!=A1) goto line_1200;
    /* BASIC: 940 H(20)=INT(100*RND(X))+1 */
line_940:
    H(20) = floor(100*((double)rand() / (double)RAND_MAX)(X))+1;
    /* BASIC: 950 IF INT(H(20)/Z)=H(20)/Z THEN 1160 */
line_950:
    if (floor(H(20)/Z)==H(20)/Z) goto line_1160;
    /* BASIC: 960 ON C GOTO 970,980 */
line_960:
    /* UNTRANSLATED: ON C GOTO 970,980 */
    /* BASIC: 970 PRINT "GOAL " A$(7):H(9)=H(9)+1:GOTO 990 */
line_970:
    printf("%g ", (double)("GOAL " A$(7))); printf("\n");
    H(9) = H(9)+1;
    goto line_990;
    /* BASIC: 980 PRINT "SCORE " B$(7):H(8)=H(8)+1 */
line_980:
    printf("%g ", (double)("SCORE " B$(7))); printf("\n");
    H(8) = H(8)+1;
    /* BASIC: 990 FOR B1=1 TO 25:PRINT CHR$(7);:NEXT B1:PRINT */
line_990:
    B1 = 1; for_loop_B1: if ((1 >= 0 && B1 > 25) || (1 < 0 && B1 < 25)) goto end_for_B1;
    printf("%g ", (double)(CHR$(7)));
    B1 += 1; goto for_loop_B1; end_for_B1:;
    printf("\n");
    /* BASIC: 1000 PRINT "SCORE: ";:IF H(8)>H(9) THEN 1020 */
line_1000:
    printf("%s", "SCORE: ");
    if (H(8)>H(9)) goto line_1020;
    /* BASIC: 1010 PRINT A$(7)":";H(9),B$(7)":";H(8):GOTO 1030 */
line_1010:
    printf("%g ", (double)(A$(7)":")); printf("%g ", (double)(H(9))); printf("\t"); printf("%g ", (double)(B$(7)":")); printf("%g ", (double)(H(8))); printf("\n");
    goto line_1030;
    /* BASIC: 1020 PRINT B$(7)":";H(8),A$(7)":";H(9) */
line_1020:
    printf("%g ", (double)(B$(7)":")); printf("%g ", (double)(H(8))); printf("\t"); printf("%g ", (double)(A$(7)":")); printf("%g ", (double)(H(9))); printf("\n");
    /* BASIC: 1030 ON C GOTO 1040,1100 */
line_1030:
    /* UNTRANSLATED: ON C GOTO 1040,1100 */
    /* BASIC: 1040 PRINT "GOAL SCORED BY: " A$(G):IF G1=0 THEN 1070 */
line_1040:
    printf("%g ", (double)("GOAL SCORED BY: " A$(G))); printf("\n");
...
line_1060:
    printf("%g ", (double)(" ASSISTED BY: " A$(G1) " && " A$(G2))); printf("\n");
    goto line_1090;
...
line_1080:
    printf("%g ", (double)(" ASSISTED BY: " A$(G1))); printf("\n");
...
line_1100:
    printf("%g ", (double)("GOAL SCORED BY: " B$(G)));
...
line_1120:
    printf("%g ", (double)(" ASSISTED BY: " B$(G1) " && " B$(G2))); printf("\n");
    goto line_1150;
...
line_1140:
    printf("%g ", (double)(" ASSISTED BY: " B$(G1))); printf("\n");
    goto line_1150;
...
line_1160:
    A2 = floor(100*((double)rand() / (double)RAND_MAX)(X))+1;
...
line_1440:
    printf("%g ", (double)("SKATE SAVE BY " A$(6))); printf("\n");
    /* BASIC: 1450 PRINT A$(6) " WHACKS THE LOOSE PUCK INTO THE STANDS" */
line_1450:
    printf("%g ", (double)(A$(6) " WHACKS THE LOOSE PUCK INTO THE STANDS")); printf("\n");
    /* BASIC: 1460 GOTO 1540 */
line_1460:
    goto line_1540;
    /* BASIC: 1470 PRINT "STICK SAVE BY " A$(6) " AND HE CLEARS IT OUT HIMSELF" */
line_1470:
    printf("%s", "STICK SAVE BY " A$(6) " AND HE CLEARS IT OUT HIMSELF"); printf("\n");
    /* BASIC: 1480 GOTO 260 */
line_1480:
    goto line_260;
    /* BASIC: 1490 PRINT "KICKED OUT BY " A$(6) */
line_1490:
    printf("%g ", (double)("KICKED OUT BY " A$(6))); printf("\n");
...
line_1520:
    printf("%s", "GLOVE SAVE " A$(6) " AND HE HANGS ON"); printf("\n");
...
line_1540:
    ...
    N = 1; for_loop_N: if ((1 >= 0 && N > 30) || (1 < 0 && N < 30)) goto end_for_N;
    printf("%g ", (double)(CHR$(7)));
    N += 1; goto for_loop_N; end_for_N:;
...
line_1600:
    { int _t; for(_t=0; _t<25; _t++) printf(" "); } printf("%g ", (double)(A$(7))); printf("\n");
...
line_1630:
    ...
    { int _t; for(_t=0; _t<5; _t++) printf(" "); } printf("%g ", (double)(A$(I))); { int _t; for(_t=0; _t<21; _t++) printf(" "); } printf("%g ", (double)(T(I))); { int _t; for(_t=0; _t<36; _t++) printf(" "); } printf("%g ", (double)(T1(I))); printf("\n");
...
line_1650:
    { int _t; for(_t=0; _t<25; _t++) printf(" "); } printf("%g ", (double)(B$(7))); printf("\n");
...
line_1680:
    T = 1; for_loop_T: if ((1 >= 0 && T > 5) || (1 < 0 && T < 5)) goto end_for_T;
    { int _t; for(_t=0; _t<5; _t++) printf(" "); } printf("%g ", (double)(B$(T))); { int _t; for(_t=0; _t<21; _t++) printf(" "); } printf("%g ", (double)(T2(T))); { int _t; for(_t=0; _t<36; _t++) printf(" "); } printf("%g ", (double)(T3(T))); printf("\n");
...
line_1700:
    ...
    printf("%g ", (double)(A$(7)":")); printf("%g ", (double)(S2)); printf("\n");
    printf("%g ", (double)(B$(7)":")); printf("%g ", (double)(S3)); printf("\n");
    /* BASIC: 1710 END */
line_1710:
    exit(0);
