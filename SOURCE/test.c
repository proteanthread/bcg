...
int main(int argc, char **argv) {
    ...
    double INCORRECT, J2, READ, P, GAME, J, NEW, TYPE, DIGITS, NICE, TOE, CONCEDES, INSTRUCTIONS, BOARD, LIKES, ANSWER, INDICATE, JERSEY, THE, DIGIT, ON, LEFT, A, DRAW, IT, PROGRAM, FOLLOWS, IN, K2, TIC, NUMBER, OUT, BY, PLAYING, MORRISTOWN, DATA, INDI, XX, ROW, J3, J4, TIME, TRY, BETWEEN, K3, WANT, LET, YOUR, SQUARES, I, INDICATED, OUTPUT, Y, M, J1, IS, K, OF, Z, PAPER, S, NICK, WILL, CREATIVE, WIN, FOX, RETYPE, MACHINE, EACH, PLEASE, YOU, WINS, K1, AS, NO, USED, SQUARE, WITH, RESPECTIVELY, ANOTHER, RUN, MENU, AN, I1, SEE, MOVES, C, MOVE, COMPUTING, JUST, GET, LEVEL, COLUMN, INCLUSIVE, L, PLACE, UNUSED, OCCUPIED, TAKES, CUBE, DO, FIRST, CATED, YES, THAT, AGAIN, QUBIC, O, TAC, N, THIS, Q, ZERO;
    double X[1000] = {0}; /* Static array fallback */
    char _input_buf[256];
...
line_230:
    if (LEFT$(C$,1)=="N") goto line_315;
    /* BASIC: 240 IF LEFT$(C$,1)="Y" THEN 265 */
line_240:
    if (LEFT$(C$,1)=="Y") goto line_265;
...
line_320:
    ...
    /* BASIC: 330 READ Y(I) */
line_330:
    /* UNTRANSLATED: READ Y(I) */
    /* BASIC: 340 NEXT I */
line_340:
    I += 1; goto for_loop_I; end_for_I:;
    /* BASIC: 350 FOR I=1 TO 76 */
line_350:
    I = 1; for_loop_I: if ((1 >= 0 && I > 76) || (1 < 0 && I < 76)) goto end_for_I;
...
line_440:
    ...
    /* BASIC: 450 INPUT S$ */
line_450:
    printf("? "); fgets(S_str, sizeof(S_str), stdin); S_str[strcspn(S_str, "\n")] = 0;
    /* BASIC: 460 IF LEFT$(S$,1)="N" THEN 630 */
line_460:
    if (LEFT$(S$,1)=="N") goto line_630;
    /* BASIC: 470 IF LEFT$(S$,1)="Y" THEN 500 */
line_470:
    if (LEFT$(S$,1)=="Y") goto line_500;
...
line_1160:
    if (L(I)==1/2) goto line_2360;
    /* BASIC: 1170 IF L(I)=1+3/8 THEN 2360 */
line_1170:
    if (L(I)==1+3/8) goto line_2360;
...
line_1440:
    if (L(I)==.5) goto line_2360;
    /* BASIC: 1450 IF L(I)=5+3/8 THEN 2360 */
line_1450:
    if (L(I)==5+3/8) goto line_2360;
...
line_1520:
    if (LEFT$(X$,1)=="Y") goto line_400;
    /* BASIC: 1530 IF LEFT$(X$,1)="N" THEN 1560 */
line_1530:
    if (LEFT$(X$,1)=="N") goto line_1560;
...
line_1550:
    goto line_1510;
    /* BASIC: 1560 STOP:RUN "MENU" */
line_1560:
    exit(0);
    /* UNTRANSLATED: RUN "MENU" */
...
line_1640:
    S = 1; for_loop_S: if ((1 >= 0 && S > 76) || (1 < 0 && S < 76)) goto end_for_S;
...
line_1830:
    K = 1; for_loop_K: if ((1 >= 0 && K > 18) || (1 < 0 && K < 18)) goto end_for_K;
    /* BASIC: 1840 LET P=0 */
line_1840:
    /* UNTRANSLATED: LET P=0 */
    /* BASIC: 1850 FOR I=4*K-3 TO 4*K */
line_1850:
    I = 4*K-3; for_loop_I: if ((1 >= 0 && I > 4*K) || (1 < 0 && I < 4*K)) goto end_for_I;
...
line_2410:
    J = A; for_loop_J: if ((5-2*A >= 0 && J > 5-A) || (5-2*A < 0 && J < 5-A)) goto end_for_J;
    /* BASIC: 2420 IF X(M(I,J))=S THEN 2450 */
line_2420:
    if (X(M(I,J))==S) goto line_2450;
    /* BASIC: 2430 NEXT J */
line_2430:
    J += 5-2*A; goto for_loop_J; end_for_J:;
...
line_2550:
    XX = 1; for_loop_XX: if ((1 >= 0 && XX > 9) || (1 < 0 && XX < 9)) goto end_for_XX;
    printf("\n");
    /* TODO: NEXT (unmatched) */
...
line_2562:
    I1 = 1; for_loop_I1: if ((1 >= 0 && I1 > J) || (1 < 0 && I1 < J)) goto end_for_I1;
    /* BASIC: 2564 PRINT"   "; */
line_2564:
    printf("%s", "   ");
    /* BASIC: 2566 NEXT I1 */
