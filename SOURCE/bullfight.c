...
int main(int argc, char **argv) {
    ...
    double ABS, INCORRECT, BETTER, FACE, READ, STILL, OLE, J, NEW, TYPE, INSTRUCTIONS, ANSWER, RE, THEY, JERSEY, THE, DON, SEVENTH, ON, PUT, FND, BULL, A, KILLED, D, BUT, IT, UNLES, AWARD, MINUTES, T, WERE, HELLO, AWFUL, SUPERB, GORED, COWARD, PANICKED, IN, NEED, CORRECT, NUMBER, DEF, MOMENT, ORDINARY, CHANCES, AFICIONADOS, ABOVE, MORRISTOWN, CHARGING, ANY, IDIOT, OUTSIDE, DATA, TOREADORES, Z1, WOULDN, FNC, EVER, WAS, POSTHUMOUSLY, HORSES, TURN, LUCK, BIG, HORNS, NOTHING, CHEST, TRY, DESERVE, DARE, HERE, WHAT, RES, ADIOS, PICADO, RING, WANT, RECIEVE, SWIRL, HOMBRE, ALL, YOUR, INSIDE, I, CHEERS, AWARDS, SHOW, CROWD, HAS, FROM, BLOODLOVERS, M, FNA, H, IS, ALIVE, K, OF, Z, DRAWN, LL, SWEAR, POOR, WILL, CREATIVE, ORES, JOB, DETERMINE, EACH, PLEASE, YOU, COMES, EARS, EAR, NO, WITH, TOREAD, PANIC, ONE, RUN, WILDLY, BOTH, LESS, GOOD, MAY, MATADOR, ARE, B, HOW, NECESSARY, C, MAKE, MOVE, TEN, VERONICA, CAPE, KILL, BRAVER, COMPUTING, TRUTH, E, STUPID, MUY, BEFORE, CHANCE, AT, L, BRAVE, FAIR, PASS, BOOS, INSTEAD, DOWN, F, DEAD, DOES, DID, DO, AGAIN, FIRST, LUCKY, YES, OVER, DANGEROUS, HAVE, Q, PICADORES;
...
line_490:
    printf("%s", "YOU HAVE DRAWN A "); printf("%g ", (double)(L$(A))); printf("%s", " BULL."); printf("\n");
...
line_570:
    ...
    /* BASIC: 590 A$="PICADO" */
line_590:
    strncpy(A_str, ""PICADO"", sizeof(A_str)); /* String assign approx */
    /* BASIC: 595 B$="RES" */
line_595:
    strncpy(B_str, ""RES"", sizeof(B_str)); /* String assign approx */
...
line_610:
    ...
    /* BASIC: 630 A$="TOREAD" */
line_630:
    strncpy(A_str, ""TOREAD"", sizeof(A_str)); /* String assign approx */
    /* BASIC: 635 B$="ORES" */
line_635:
    strncpy(B_str, ""ORES"", sizeof(B_str)); /* String assign approx */
...
line_700:
    printf("%s", "PASS NUMBER"); printf("%g ", (double)(D(3))); printf("\n");
    /* BASIC: 710 IF D(3)<3 THEN 760 */
line_710:
    if (D(3)<3) goto line_760;
...
line_940:
    F = (6-A+M/10)*((double)rand() / (double)RAND_MAX)/((D(1)+D(2)+D(3)/10)*5);
...
line_970:
    /* UNTRANSLATED: ON FNA(0) GOTO 980,1010 */
...
line_1080:
    /* UNTRANSLATED: ON FNA(0) GOTO 1090,1110 */
    /* BASIC: 1090 D(4)=2 */
line_1090:
    D(4) = 2;
    /* BASIC: 1100 GOTO 660 */
...
line_1230:
    K = (6-A)*10*((double)rand() / (double)RAND_MAX)/((D(1)+D(2))*5*D(3));
...
line_1760:
    printf("%s", "THE "); printf("%s", A_str); printf("%s", B_str); printf("%s", " DID A "); printf("%g ", (double)(L$(T))); printf("%s", " JOB."); printf("\n");
...
line_1790:
    /* UNTRANSLATED: ON FNA(K) GOTO 1830,1850 */
    /* BASIC: 1800 IF A$="TOREAD" THEN 1820 */
line_1800:
    if (A$=="TOREAD") goto line_1820;
...
line_1940:
    if (A$=="YES") goto line_1990;
    /* BASIC: 1950 IF A$="NO" THEN 2010 */
line_1950:
    if (A$=="NO") goto line_2010;
