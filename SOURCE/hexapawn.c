...
int main(int argc, char **argv) {
    ...
    double WINNING, ENTER, READ, ALWAYS, IMPOSSIBLE, P, GAME, GO, RANDOMLY, KNOWING, TOLD, J, NEW, ORDINATES, PAWNS, TYPE, SO, INSTRUCTIONS, SERIES, BOARD, FNR, JERSEY, THE, PLAYED, GAMES, ADVANTAGE, MOVING, STARTS, M1, ON, LEFT, A, PLAYS, EXAMPLE, WHEN, DRAW, WON, IT, BUT, DIFFICULT, PROGRAM, SPORT, T, M2, FOLLOWS, IN, NUMBER, DEF, OUT, OPPOSING, FNS, BY, PLAYING, MORRISTOWN, MID, RIGHTMOST, DATA, MUST, HOWEVER, HEXAPAWN, CAPTURE, ILLEGAL, OFFSET, FNM, SPACE, COMMA, SINCE, CO, NUMBERS, STRATEGY, FOLLOWED, YOUR, I, SQUARES, RESPONSE, MAN, HAS, FROM, Y, M, MORE, INITIAL, W, LEARNS, MOVED, IS, K, EMPTY, OF, ONLY, LL, S, WILL, THUS, CREATIVE, WIN, COMPUTER, EACH, YOU, X, BECOMES, AS, LEARN, NO, STR, FORWARD, SQUARE, WITH, WOULD, ONE, QUESTION, GOOD, AN, CHESS, ARE, BE, MOVES, HOW, MOVE, RESIGN, CAN, NUMBERING, COMPUTING, JUST, HELP, PATTERN, AT, L, DIAGONALLY, ALSO, FIRST, R, PAWN, O, SEPERATED, N, HAVE, THIS;
...
line_6:
    ...
    /* BASIC: 7 REM  ORIGINAL VERSION FOR H-P TIMESHARE SYSTEM BY R.A. KAAPKE 5/5/76 */
line_7:
    // REM  ORIGINAL VERSION FOR H-P TIMESHARE SYSTEM BY R.A. KAAPKE 5/5/76
    /* BASIC: 8 REM  INSTRUCTIONS BY JEFF DALTON */
line_8:
    // REM  INSTRUCTIONS BY JEFF DALTON
    /* BASIC: 9 REM  CONVERSION TO MITS BASIC BY STEVE NORTH */
line_9:
...
line_30:
    ...
    /* BASIC: 35 P$="X.O" */
line_35:
    strncpy(P_str, ""X.O"", sizeof(P_str)); /* String assign approx */
    /* BASIC: 40 FOR I=1 TO 19: FOR J=1 TO 9: READ B(I,J): NEXT J: NEXT I */
line_40:
    I = 1; for_loop_I: if ((1 >= 0 && I > 19) || (1 < 0 && I < 19)) goto end_for_I;
    J = 1; for_loop_J: if ((1 >= 0 && J > 9) || (1 < 0 && J < 9)) goto end_for_J;
    /* UNTRANSLATED: READ B(I,J) */
...
line_121:
    printf("? "); if (fgets(_input_buf, sizeof(_input_buf), stdin)) sscanf(_input_buf, "%lf", &M1,M2);
    /* BASIC: 122 IF M1=INT(M1)AND M2=INT(M2)AND M1>0 AND M1<10 AND M2>0 AND M2<10 THEN 130 */
line_122:
    if (M1==floor(M1)AND M2==floor(M2)AND M1>0 && M1<10 && M2>0 && M2<10) goto line_130;
...
line_240:
    if (S(I)!=-1) goto line_330;
    /* BASIC: 250 IF S(I+3)=0 THEN 350 */
line_250:
    if (S(I+3)==0) goto line_350;
...
line_280:
    if (S(5)==1) goto line_350;
    /* BASIC: 290 GOTO 330 */
line_290:
    goto line_330;
    /* BASIC: 300 IF S(8)=1 THEN 350 */
line_300:
    if (S(8)==1) goto line_350;
    /* BASIC: 310 GOTO 330 */
line_310:
    goto line_330;
    /* BASIC: 320 IF S(I+2)=1 OR S(I+4)=1 THEN 350 */
line_320:
    if (S(I+2)==1 || S(I+4)==1) goto line_350;
    /* BASIC: 330 NEXT I */
line_330:
    I += 1; goto for_loop_I; end_for_I:;
    /* BASIC: 340 GOTO 820 */
...
line_370:
    K = 3; for_loop_K: if ((-1 >= 0 && K > 1) || (-1 < 0 && K < 1)) goto end_for_K;
...
line_601:
    if (M(X,Y)==0) goto line_600;
    /* BASIC: 610 IF R<>0 THEN 630 */
line_610:
    if (R!=0) goto line_630;
    /* BASIC: 620 PRINT "I MOVE FROM ";STR$(INT(M(X,Y)/10));" TO ";STR$(FNM(M(X,Y))) */
line_620:
    printf("%s", "I MOVE FROM "); printf("%g ", (double)(STR$(floor(M(X)); printf("\t"); printf("%g ", (double)(Y)/10)))); printf("%s", " TO "); printf("%g ", (double)(STR$(FNM(M(X)); printf("\t"); printf("%g ", (double)(Y))))); printf("\n");
...
line_630:
    printf("%s", "I MOVE FROM "); printf("%g ", (double)(STR$(FNR(floor(M(X)); printf("\t"); printf("%g ", (double)(Y)/10))))); printf("%s", " TO ");
    /* BASIC: 631 PRINT STR$(FNR(FNM(M(X,Y)))) */
line_631:
    printf("%g ", (double)(STR$(FNR(FNM(M(X)); printf("\t"); printf("%g ", (double)(Y)))))); printf("\n");
...
line_641:
    if (S(7)==-1 || S(8)==-1 || S(9)==-1) goto line_870;
    /* BASIC: 650 FOR I=1 TO 9 */
line_650:
    I = 1; for_loop_I: if ((1 >= 0 && I > 9) || (1 < 0 && I < 9)) goto end_for_I;
    /* BASIC: 660 IF S(I)=1 THEN 690 */
line_660:
    if (S(I)==1) goto line_690;
    /* BASIC: 670 NEXT I */
line_670:
    I += 1; goto for_loop_I; end_for_I:;
    /* BASIC: 680 GOTO 870 */
...
line_700:
    if (S(I)!=1) goto line_790;
    /* BASIC: 710 IF S(I-3)=0 THEN 120 */
line_710:
    if (S(I-3)==0) goto line_120;
...
line_740:
    if (S(5)==-1) goto line_120;
    /* BASIC: 750 GOTO 790 */
line_750:
    goto line_790;
    /* BASIC: 760 IF S(2)=-1 THEN 120 */
line_760:
    if (S(2)==-1) goto line_120;
    /* BASIC: 770 GOTO 790 */
line_770:
    goto line_790;
    /* BASIC: 780 IF S(I-2)=-1 OR S(I-4)=-1 THEN 120 */
line_780:
    if (S(I-2)==-1 || S(I-4)==-1) goto line_120;
    /* BASIC: 790 NEXT I */
line_790:
    I += 1; goto for_loop_I; end_for_I:;
...
line_1030:
    { int _t; for(_t=0; _t<10; _t++) printf(" "); } printf("%s", MID$(P_str); printf("\t"); printf("%g ", (double)(S((I-1)*3+J)+2)); printf("\t"); printf("%g ", (double)(1)));
    /* BASIC: 1040 NEXT J */
line_1040:
    J += 1; goto for_loop_J; end_for_J:;
    /* BASIC: 1050 PRINT */
