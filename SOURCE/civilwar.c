...
int main(int argc, char **argv) {
    ...
    double EXACT, FACTS, JACKSON, MEN, BEAUREGARD, TEND, ACTUAL, MARYLAND, THEY, ARTILLERY, FORCED, IT, WON, RICHMOND, DEALT, NUMBER, SIMULATION, GENERAL, BY, CONFEDERACY, THEM, WAR, CHICKAMAUGA, BECAUSE, USUALLY, BROKE, ALL, SALARIES, SURRENDERED, M, IS, GRANT, OF, ONLY, SUCCESSFULLY, OBJECT, SAME, DEPENDS, COMPUTER, REQUEST, NO, SPEND, WITH, ONE, OTHER, AFTER, FAIL, W0, HOW, C, HELPED, RERUNS, PRESENT, TYPING, TWO, M6, THROUGHOUT, GEN, ENTRIES, ACCESS, O, ISOLATED, DESERTIONS, S0, GAME, NEW, TYPE, FIGURES, POSSIBLE, COLD, INSTRUCTIONS, APRIL, PREVIOUS, FOREST, A, INFLATION, CHANGE, BUT, VICTORY, GETTYSBURG, CHANCELLORSVILLE, MORALE, SITUATION, ANY, SPOTSYLVANIA, MCDOWELL, BURNSIDE, LUCK, Y2, HERE, ALLOWING, COMMANDING, U, DESCRIPTIONS, WANT, ENTRY, LET, SURPRISE, YOUR, I, MISSISSIPPI, Q2, HAS, FALLING, H, M5, SIMULATE, MUCH, MET, ARMIES, COMBINED, POOR, LED, C2, SIEGE, YOU, NOTE, FORCES, AS, ASKS, RUN, BATTLE, NEAR, OFFENSIVE, AN, MAY, SHILOH, MANEUVERS, INTO, AUGUST, ALLOCATIONS, ATLANTA, MOST, ATTACK, E, SOUTH, AT, AGAIN, THAT, YES, BRINGS, N, HISTORY, USE, ABS, P, CONVERGED, SIDES, ANSWER, REPLAY, THE, INTELLIGENCE, M1, ON, DEFEAT, ROSENCRANS, FORTIFICATION, REPULSED, M2, SURRENDER, C6, RESULT, ENCIRCLEMENT, MORRISTOWN, BEGAN, DATA, FREDERICKSBURG, EXCUDING, ARMY, LEE, ANTIETAM, ASSUME, THREE, C1, JULY, SECOND, AMMUNITION, S2, RESPONSE, C5, COST, O2, PERCENTAGES, UNION, U2, ORGANIZATION, CONFEDERATE, FOUGHT, NEGATIVE, DUE, SELECT, I2, LOSE, X, Y1, COSTLY, COMPARED, INCORPORATE, SEVEN, T1, CAUSES, PLAN, FAILED, THEIR, STONEWALL, WISH, NOV, INSTANT, WHICH, FRONTAL, MANY, BATTLES, FOLLOWING, FAIR, DID, GENERALS, SOUTHERN, HAVE, M4, OUTCOME, ENTER, READ, SHERMAN, CONFUSION, P2, DAYS, JERSEY, M3, AWAY, UNRESOLVED, BULL, D, WHEN, PROGRAM, MONTHS, WERE, VETERAN, R1, HARBOR, HAD, BRAGG, IN, JUNE, CRUCIAL, AUG, KEEP, UNDER, SUGGESTS, STRATEGIES, WAS, MONEY, WASHINGTON, PLAY, BASED, Q1, BACK, F1, E2, CLOSE, RESPONSES, OCCURRENCE, UPHELD, R2, STRATEGY, DEFENSIVE, COULD, THERE, DROVE, FROM, Y, VICKSBURG, CIVIL, W, MISTAKE, SIEGED, CHOICES, Z, CREATIVE, WIN, LOSSES, FLANKING, THINK, OUTSTANDING, MCCLELLAN, CONTINUED, CASUALTIES, WINS, DEC, GAVE, USED, HIGH, BOTH, SEPT, GOOD, P1, CHATTANOOGA, I1, FOOD, HISTORICAL, ARE, MURFREESBORO, CSA, B, AGAINST, BLOW, LOST, ORIGINAL, COMPUTING, VALUES, DEATH, PUSH, SIMULATED, L, F, DO, REQUESTING, R, PETERSBURG, T2, THIS, ALLOWED;
...
line_2:
    { int _t; for(_t=0; _t<26) "CIVIL WAR; _t++) printf(" "); } printf("\n");
...
line_6:
    ...
    printf("\n");
    printf("\n");
    /* BASIC: 20 REM ORIGINAL GAME DESIGN: CRAM, GOODIE, HIBBARD LEXINGTON H.S. */
line_20:
    // REM ORIGINAL GAME DESIGN
    /* UNTRANSLATED: CRAM, GOODIE, HIBBARD LEXINGTON H.S. */
...
line_70:
    ...
    S(3) = 25;
    S(4) = 25;
    /* BASIC: 82  REM READ HISTORICAL DATA. */
line_82:
    // REM READ HISTORICAL DATA.
    /* BASIC: 84  FOR D=1 TO 14 */
line_84:
    D = 1; for_loop_D: if ((1 >= 0 && D > 14) || (1 < 0 && D < 14)) goto end_for_D;
    /* BASIC: 86  READ C$(D),M1(D),M2(D),C1(D),C2(D),M(D) */
line_86:
    /* UNTRANSLATED: READ C$(D),M1(D),M2(D),C1(D),C2(D),M(D) */
    /* BASIC: 88  NEXT D */
line_88:
...
line_120:
    if (X$=="YES") goto line_160;
    /* BASIC: 130  IF X$="NO" THEN 370 */
line_130:
    if (X$=="NO") goto line_370;
...
line_400:
    if (B$=="YES") goto line_430;
    /* BASIC: 410  IF B$ <> "NO" THEN 380 */
line_410:
    if (B$ != "NO") goto line_380;
...
line_580:
    if (X$=="YES") goto line_600;
    /* BASIC: 590  IF X$ <> "NO" THEN 560 */
line_590:
    if (X$ != "NO") goto line_560;
...
line_1125:
    /* UNTRANSLATED: ON A GOTO 3580,3620,3650,3690,3720,3750,3780,3800,3830,3860,3890 */
    /* BASIC: 1130  ON A-11 GOTO 3920,3950,3980 */
line_1130:
    /* UNTRANSLATED: ON A-11 GOTO 3920,3950,3980 */
    /* BASIC: 1140  PRINT C$" INSTANT REPLAY" */
line_1140:
    printf("%g ", (double)(C$" INSTANT REPLAY")); printf("\n");
    /* BASIC: 1150  PRINT */
line_1150:
    printf("\n");
...
line_1170:
    printf("%s", "MEN"); printf("\t"); printf("%g ", (double)(" "M5)); printf("\t"); printf("%g ", (double)(" "M6)); printf("\n");
    /* BASIC: 1180  PRINT "MONEY","$";D(1),"$";D(2) */
line_1180:
    printf("%s", "MONEY"); printf("\t"); printf("%s", "$"); printf("%g ", (double)(D(1))); printf("\t"); printf("%s", "$"); printf("%g ", (double)(D(2))); printf("\n");
...
line_1195:
    printf("\n");
    /* BASIC: 1200  REM - ONLY IN PRINTOUT IS CONFED INFLATION = I1+15% */
line_1200:
...
line_1220:
    I = 1; for_loop_I: if ((1 >= 0 && I > D) || (1 < 0 && I < D)) goto end_for_I;
    /* BASIC: 1230  IF B$ <> "YES" THEN 1260 */
line_1230:
    if (B$ != "YES") goto line_1260;
...
line_1380:
    printf("? "); if (fgets(_input_buf, sizeof(_input_buf), stdin)) sscanf(_input_buf, "%lf", &H(I));
    /* BASIC: 1390  LET N=1 */
line_1390:
    /* UNTRANSLATED: LET N=1 */
    /* BASIC: 1400  IF H(I)<0 THEN 1490 */
line_1400:
    if (H(I)<0) goto line_1490;
    /* BASIC: 1410  PRINT " - AMMUNITION"; */
line_1410:
    printf("%s", " - AMMUNITION");
    /* BASIC: 1420  INPUT B(I) */
line_1420:
    printf("? "); if (fgets(_input_buf, sizeof(_input_buf), stdin)) sscanf(_input_buf, "%lf", &B(I));
    /* BASIC: 1430  LET N=2 */
line_1430:
    /* UNTRANSLATED: LET N=2 */
    /* BASIC: 1440  IF B(I)<0 THEN 1490 */
line_1440:
    if (B(I)<0) goto line_1490;
    /* BASIC: 1450  PRINT */
line_1450:
    printf("\n");
    /* BASIC: 1460  IF F(I)+H(I)+B(I) <= D(I) THEN 1510 */
line_1460:
    if (F(I)+H(I)+B(I) <== D(I)) goto line_1510;
    /* BASIC: 1470  PRINT "THINK AGAIN! YOU HAVE ONLY $"D(I) */
line_1470:
    printf("%g ", (double)("THINK AGAIN! YOU HAVE ONLY $"D(I))); printf("\n");
...
line_1500:
    /* UNTRANSLATED: ON N GOTO 1370,1410 */
    /* BASIC: 1510  IF B$ <> "YES" THEN 1550 */
line_1510:
    if (B$ != "YES") goto line_1550;
...
line_1550:
    Z = 1; for_loop_Z: if ((1 >= 0 && Z > D) || (1 < 0 && Z < D)) goto end_for_Z;
    /* BASIC: 1560  IF B$ <> "YES" THEN 1620 */
line_1560:
    if (B$ != "YES") goto line_1620;
    /* BASIC: 1570  ON Z GOTO 1580,1600 */
line_1570:
    /* UNTRANSLATED: ON Z GOTO 1580,1600 */
...
line_1860:
    if (B$ != "YES") goto line_1910;
    /* BASIC: 1870  FOR I=1 TO 2 */
line_1870:
    I = 1; for_loop_I: if ((1 >= 0 && I > 2) || (1 < 0 && I < 2)) goto end_for_I;
    /* BASIC: 1880  ON I GOTO 1890,1920 */
line_1880:
    /* UNTRANSLATED: ON I GOTO 1890,1920 */
...
line_2060:
    ...
    /* BASIC: 2070  REM : SIMULATED LOSSES-NORTH */
line_2070:
    // REM
    /* UNTRANSLATED: SIMULATED LOSSES-NORTH */
...
line_2310:
    if (B$ != "YES") goto line_2350;
    /* BASIC: 2320  PRINT "COMPARED TO THE ACTUAL CASUALTIES AT "C$ */
line_2320:
    printf("%s", "COMPARED TO THE ACTUAL CASUALTIES AT "C_str); printf("\n");
    /* BASIC: 2330  PRINT "CONFEDERATE:"INT(100*(C5/C1)+.5)"% OF THE ORIGINAL" */
line_2330:
    printf("%s", "CONFEDERATE:"INT(100*(C5/C1)+.5)"% OF THE ORIGINAL"); printf("\n");
    /* BASIC: 2340  PRINT "UNION:      "INT(100*(C6/C2)+.5)"% OF THE ORIGINAL" */
line_2340:
    printf("%s", "UNION:      "INT(100*(C6/C2)+.5)"% OF THE ORIGINAL"); printf("\n");
    /* BASIC: 2350  PRINT */
line_2350:
    printf("\n");
...
line_2420:
    printf("%s", "THE UNION WINS "C_str); printf("\n");
    /* BASIC: 2430  GOTO 2600 */
line_2430:
    goto line_2600;
    /* BASIC: 2440  PRINT "THE CONFEDERACY WINS "C$ */
line_2440:
    printf("%s", "THE CONFEDERACY WINS "C_str); printf("\n");
...
line_2530:
    printf("%s", "YOUR CASUALTIES WERE "INT(100*(C5/C1)+.5)"% OF "); printf("\n");
...
line_2630:
    ...
    /* BASIC: 2640  REM - CUMULATIVE BATTLE FACTORS WHICH ALTER HISTORICAL */
line_2640:
...
line_2880:
    printf("%g ", (double)("HAS WON "W" BATTLES && LOST "L)); printf("\n");
...
line_2970:
    printf("%s", "FOR THE "W+L+W0" BATTLES FOUGHT (EXCUDING RERUNS)"); printf("\n");
    /* BASIC: 2980  PRINT " "," "," "; */
line_2980:
...
line_3080:
    printf("%g ", (double)(S(1))); printf("%g ", (double)(S2(2))); printf("%g ", (double)(S(3))); printf("%g ", (double)(S(4))); printf("\n");
    /* BASIC: 3090  REM--------------------------------- */
line_3090:
    // REM---------------------------------
    /* BASIC: 3100  STOP */
...
line_3290:
    /* TODO: RETURN */
    /* BASIC: 3300  REM LEARN  PRESENT STRATEGY, START FORGETTING OLD ONES */
line_3300:
...
line_3410:
    /* TODO: RETURN */
    /* BASIC: 3420  REM - HISTORICAL DATA...CAN ADD MORE (STRAT.,ETC) BY INSERTING */
line_3420:
