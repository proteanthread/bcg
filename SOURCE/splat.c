...
int main(int argc, char **argv) {
    ...
    double FALL, SATURN, GOING, BETTER, THAN, GAME, GO, CHUTES, SON, MERCURY, J, NEW, RANKED, POSSIBLE, URANUS, ALTITUDE, GRAVITY, JUMP, PANTS, OK, RE, JUMPING, SQR, JERSEY, THE, HEY, MUTTER, DON, SIMULATES, ON, JUPITER, SSSSSSSSSS, D, PARACHUTE, RIGHT, WOW, UP, IT, SPORTING, A1, REST, PICKED, SUN, T, PACE, WERE, GUN, ANGEL, IN, NUMBER, OUT, MOMENT, FT, HMMM, BY, ACCELERATION, MORRISTOWN, MOON, DIST, AMAZING, ANY, SOON, PARADISE, HEADED, HEAVEN, CAME, PLAY, THEM, SECONDS, TIME, TRY, PUSHING, CONSERVATIVE, EARTH, HERE, WHAT, BAD, YOURS, BLOOD, V, VELOCITY, WANT, OPEN, TOO, DAISIES, ALL, YOUR, I, SHORTER, THERE, M, NEPTUNE, WITHOUT, BEEN, IS, TIMER, OWN, K, KICK, OF, ONLY, MUCH, B1, LL, Z, SET, S, OPENED, PRECEDED, DUE, CREATIVE, SHAPE, SELECT, LOWER, CORD, PLEASE, YOU, REQUIESCAT, K1, SHOULD, NO, BEATEN, TERMINAL, WITH, EASY, JUMPS, HUMPH, ANOTHER, WE, MI, LAST, GOOD, BOOST, A2, GOT, MAY, THEIR, PULLED, COME, V1, PLUS, HOW, INTO, FREEFALL, LEAD, TERM, SEC, COMPUTING, REACHED, GET, SOME, RIP, WELCOME, GIVE, CHUTE, SPLAT, MANY, MARS, BEFORE, CHANCE, AT, AREN, PEACE, WORST, DID, DO, AGAIN, THAT, YES, SUCCESSFUL, HR, VENUS, D1, N, HAVE, FINE, PRETTY;
...
line_119:
    ...
    printf("? "); fgets(A1_str, sizeof(A1_str), stdin); A1_str[strcspn(A1_str, "\n")] = 0;
    /* BASIC: 120 IF A1$="NO" THEN 128 */
line_120:
    if (A1$=="NO") goto line_128;
    /* BASIC: 121 IF A1$<>"YES" THEN PRINT "YES OR NO";:INPUT A1$:GOTO 120 */
line_121:
    if (A1$!="YES") { printf("%s", "YES OR NO"); }
    printf("? "); fgets(A1_str, sizeof(A1_str), stdin); A1_str[strcspn(A1_str, "\n")] = 0;
    goto line_120;
...
line_130:
    printf("%s", "OK.  TERMINAL VELOCITY ="V1"MI/HR"); printf("\n");
...
line_135:
    ...
    /* BASIC: 136 INPUT B1$ */
line_136:
    printf("? "); fgets(B1_str, sizeof(B1_str), stdin); B1_str[strcspn(B1_str, "\n")] = 0;
    /* BASIC: 140 IF B1$="NO" THEN 150 */
line_140:
    if (B1$=="NO") goto line_150;
    /* BASIC: 141 IF B1$<>"YES" THEN PRINT "YES OR NO";:INPUT B1$:GOTO 140 */
line_141:
    if (B1$!="YES") { printf("%s", "YES OR NO"); }
    printf("? "); fgets(B1_str, sizeof(B1_str), stdin); B1_str[strcspn(B1_str, "\n")] = 0;
    goto line_140;
...
line_206:
    printf("%s", "    ALTITUDE         ="D1"FT"); printf("\n");
    /* BASIC: 207 PRINT "    TERM. VELOCITY   ="V1"FT/SEC +/-5%" */
line_207:
    printf("%s", "    TERM. VELOCITY   ="V1"FT/SEC +/-5%"); printf("\n");
    /* BASIC: 208 PRINT "    ACCELERATION     ="A2"FT/SEC/SEC +/-5%" */
line_208:
    printf("%s", "    ACCELERATION     ="A2"FT/SEC/SEC +/-5%"); printf("\n");
...
line_300:
    I = 0; for_loop_I: if (((T/8) >= 0 && I > T) || ((T/8) < 0 && I < T)) goto end_for_I;
...
line_340:
    ...
    /* BASIC: 350 NEXT I */
line_350:
    I += (T/8); goto for_loop_I; end_for_I:;
    /* BASIC: 360 GOTO 500 */
line_360:
    goto line_500;
    /* BASIC: 400 PRINT "TERMINAL VELOCITY REACHED AT T PLUS"V/A"SECONDS." */
line_400:
    printf("%s", "TERMINAL VELOCITY REACHED AT T PLUS"V/A"SECONDS."); printf("\n");
    /* BASIC: 405 FOR I=I TO T STEP (T/8) */
line_405:
    I = I; for_loop_I: if (((T/8) >= 0 && I > T) || ((T/8) < 0 && I < T)) goto end_for_I;
    /* BASIC: 410 D=D1-((V^2/(2*A))+(V*(I-(V/A)))) */
line_410:
    D = D1-((V^2/(2*A))+(V*(I-(V/A))));
...
line_550:
    J = 0; for_loop_J: if ((1 >= 0 && J > 42) || (1 < 0 && J < 42)) goto end_for_J;
    /* BASIC: 555 IF A(J)=0 THEN 620 */
line_555:
    if (A(J)==0) goto line_620;
    /* BASIC: 560 K=K+1 */
line_560:
    K = K+1;
    /* BASIC: 570 IF D>=A(J) THEN 600 */
line_570:
    if (D>==A(J)) goto line_600;
    /* BASIC: 580 K1=K1+1 */
line_580:
    K1 = K1+1;
    /* BASIC: 600 NEXT J */
...
line_700:
    printf("%s", "WOW!  THAT'S SOME JUMPING.  OF THE"K"SUCCESSFUL JUMPS"); printf("\n");
    /* BASIC: 701 PRINT "BEFORE YOURS, ONLY"K-K1"OPENED THEIR CHUTES LOWER THAN" */
line_701:
    printf("%s", "BEFORE YOURS, ONLY"K-K1"OPENED THEIR CHUTES LOWER THAN"); printf("\n");
...
line_710:
    printf("%s", "PRETTY GOOD! " K"SUCCESSFUL JUMPS PRECEDED YOURS AND ONLY"); printf("\n");
    /* BASIC: 711 PRINT K-K1" OF THEM GOT LOWER THAN YOU DID BEFORE THEIR CHUTES" */
line_711:
    printf("%g ", (double)(K-K1" OF THEM GOT LOWER THAN YOU DID BEFORE THEIR CHUTES")); printf("\n");
...
line_720:
    printf("%s", "NOT BAD.  THERE HAVE BEEN"K"SUCCESSFUL JUMPS BEFORE YOURS."); printf("\n");
    /* BASIC: 721 PRINT"YOU WERE BEATEN OUT BY"K-K1"OF THEM.":GOTO 2000 */
line_721:
    printf("%s", "YOU WERE BEATEN OUT BY"K-K1"OF THEM."); printf("\n");
    goto line_2000;
    /* BASIC: 730 PRINT "CONSERVATIVE, AREN'T YOU?  YOU RANKED ONLY"K-K1"IN THE" */
line_730:
    printf("%s", "CONSERVATIVE, AREN'T YOU?  YOU RANKED ONLY"K-K1"IN THE"); printf("\n");
    /* BASIC: 731 PRINT K"SUCCESSFUL JUMPS BEFORE YOURS.":GOTO 2000 */
line_731:
    printf("%g ", (double)(K"SUCCESSFUL JUMPS BEFORE YOURS.")); printf("\n");
    goto line_2000;
...
line_741:
    printf("%g ", (double)(K"SUCCESSFUL JUMPS BEFORE YOURS && YOU CAME IN"K1"JUMPS")); printf("\n");
...
line_750:
    printf("%s", "HEY!  YOU PULLED THE RIP CORD MUCH TOO SOON.  "K"SUCCESSFUL"); printf("\n");
    /* BASIC: 751 PRINT "JUMPS BEFORE YOURS AND YOU CAME IN NUMBER"K-K1"!  GET WITH IT!" */
line_751:
    printf("%s", "JUMPS BEFORE YOURS AND YOU CAME IN NUMBER"K-K1"!  GET WITH IT!"); printf("\n");
    /* BASIC: 752 GOTO 2000 */
line_752:
    goto line_2000;
...
line_1010:
    printf("%g ", (double)((V/A)+((D1-(V^2/(2*A)))/V))); printf("\t"); printf("%s", "SPLAT"); printf("\n");
...
line_2001:
    if (Z$=="YES") goto line_118;
    /* BASIC: 2002 IF Z$="NO" THEN 2005 */
line_2002:
    if (Z$=="NO") goto line_2005;
